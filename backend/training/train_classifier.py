...
def train(data_dir, epochs=20, batch_size=32, lr=1e-4, save_path="organ_classifier.pth", device='cuda'):
    class_map = {cls: idx for idx, cls in enumerate(ORGAN_CLASSES)}
    ...
    model = load_organ_classifier(device=device)
