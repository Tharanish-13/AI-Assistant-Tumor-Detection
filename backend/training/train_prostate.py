...
def train(data_dir, epochs=20, batch_size=32, lr=1e-4, save_path="prostate_model.pth", device='cuda'):
    class_map = {cls: idx for idx, cls in enumerate(PROSTATE_CLASSES)}
    ...
    model = load_prostate_model(device=device)
    ...
    for epoch in range(epochs):
        ...
        for inputs, labels in train_loader:
            ...
            labels_one_hot = torch.zeros(labels.size(0), len(PROSTATE_CLASSES)).to(device)
