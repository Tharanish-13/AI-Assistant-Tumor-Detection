...
def train(data_dir, epochs=20, batch_size=32, lr=1e-4, save_path="lung_model.pth", device='cuda'):
    class_map = {cls: idx for idx, cls in enumerate(LUNG_CLASSES)}
    ...
    # Lung uses both classification and localization, focusing on classification here
    model, _ = load_lung_model(device=device) 
    ...
    for epoch in range(epochs):
        ...
        for inputs, labels in train_loader:
            ...
            labels_one_hot = torch.zeros(labels.size(0), len(LUNG_CLASSES)).to(device)
